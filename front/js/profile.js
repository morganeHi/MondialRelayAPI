...
//! ################################# Get informations and display
(async () => {
	const { data } = await axios('https://shielded-escarpment-13682.herokuapp.com/user/me', {
	...
	if (data.favoritePR) {
		if (data.favoritePR !== 0) {
			let { data: pointRelais } = await axios.post('https://shielded-escarpment-13682.herokuapp.com/mr/search', {
				country: 'FR',
				results: '1',
...
document.getElementById('modifyButton').addEventListener('click', event => {
	...
	document.getElementById('saveButton').addEventListener('click', async e => {
		...
		const { data } = await axios.put('https://shielded-escarpment-13682.herokuapp.com/user/me', body, {
			headers: { Authorization: 'Bearer ' + localStorage.getItem('Authorization') }
		});
